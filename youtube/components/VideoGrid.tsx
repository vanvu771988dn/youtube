...
const VideoGrid: React.FC<VideoGridProps> = ({ videos, loading, error, hasMore, loadMore, refresh, mode, onSimilarChannel }) => {
  ...
  return (
    <div>
      ...
      {/* Load More button - show when hasMore is true, regardless of current video count */}
      {hasMore && (
        <div className="mt-8 mb-8 text-center">
          ...
          >
            {loading ? (
              ...
              `Load More${videos.length < 10 ? ' (Filtered Results)' : ''}`
            )}
          </button>
          {videos.length < 10 && (
            <p className="text-sm text-slate-400 mt-2">
              Fewer results due to active filters. More content may be available.
            </p>
          )}
        </div>
      )}
